...
def allInputs(commandInputs: adsk.core.CommandInputs):
	"""
	Yields every `CommandInput`, including those nested inside `GroupCommandInput`s.
	"""
	for input in commandInputs:
		yield input
		if isinstance(input, adsk.core.GroupCommandInput):
			yield from allInputs(input.children)
